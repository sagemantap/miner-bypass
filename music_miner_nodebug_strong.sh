...
# Super anti disconnect (monitor jaringan dan route)
anti_disconnect_super() {
    while true; do
        if ! ping -c1 8.8.8.8 >/dev/null 2>&1; then
            echo "[!] Network lost @$(date), restarting network stack (non-root fallback)" | tee -a "$LOGFILE"
            curl -s --max-time 10 https://1.1.1.1 >/dev/null 2>&1 || true
            sleep 5
        fi
        sleep 30
    done
    ...
    while true; do
        ...
        while sleep 25; do
            if ! ps -p $PID > /dev/null; then
                ...
                kill $KPID $FPID $ASPID $NPID 2>/dev/null || true
                sleep 3
                break
            fi
        done
