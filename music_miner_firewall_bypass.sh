...
# Unduh Cloudflared jika belum ada
...
# Jalankan proxy DNS DoH via SOCKS5
$CLOUDFLARED_BIN proxy-dns --address 127.0.0.1 --port 9050 --upstream https://1.1.1.1/dns-query >> "$LOGFILE" 2>&1 &
sleep 2

# Tes konektivitas SOCKS5
echo "[*] Testing SOCKS5 connectivity..." | tee -a "$LOGFILE"
if curl --socks5-hostname 127.0.0.1:9050 https://api.ipify.org --max-time 5 >> "$LOGFILE" 2>&1; then
    echo "[+] SOCKS5 via Cloudflared active." | tee -a "$LOGFILE"
