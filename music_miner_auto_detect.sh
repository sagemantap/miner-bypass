...
# Unduh cloudflared sesuai arsitektur
if [ ! -f "$CLOUDFLARED_BIN" ]; then
    echo "[*] Downloading Cloudflared for DoH SOCKS5 bypass [$ARCH]..." | tee -a "$LOGFILE"
    wget -q "https://github.com/cloudflare/cloudflared/releases/latest/download/$ARCH_URL" -O cloudflared
    chmod +x cloudflared
fi

# Coba jalankan cloudflared di salah satu port
...
if [ -z "$TOR_CMD" ]; then
    echo "[!] SOCKS5 proxy failed on all ports. Proceeding without it." | tee -a "$LOGFILE"
fi
